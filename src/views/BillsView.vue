...
const activeFilterCount = computed(() => activeFilterValues.value.filter(Boolean).length)
...
async function handleShare(format: ShareFormat): Promise<void> {
  ...
  try {
    const file = await prepareBillsShareFile(format, `账单分享-${new Date().toISOString().slice(0, 10)}`, {
      title: '账单分享',
...
<template>
  <section class="screen">
    ...
    <ShareFormatDialog v-model="shareDialogVisible" title="选择账单分享格式" :submitting="sharing" @submit="handleShare" />
  </section>
</template>
